...
import { SarvamVoiceRecorder, playSarvamTTS, stopSarvamTTS, checkSarvamStatus } from '../../utils/sarvamAudio';
...
export default function WeatherGPTChat({ currentLocation, onNavigateToAlerts }) {
  ...
  // Check Sarvam AI backend service availability on mount
  useEffect(() => {
    checkSarvamStatus().then((status) => {
      if (status && status.enabled) {
        setSarvamAvailable(true);
        console.log('[Sarvam AI] Voice Service Active on backend');
      }
    });
  }, []);
  ...
  // Handle Sarvam Speech-to-Text Voice Recording
  const handleSarvamRecordToggle = async () => {
    setVoiceInputNotice(null);

    if (recordingState === 'idle') {
      try {
        sarvamRecorderRef.current = new SarvamVoiceRecorder();
        await sarvamRecorderRef.current.startRecording();
        setRecordingState('recording');
      } catch (err) {
        setRecordingState('idle');
        console.warn('[Sarvam STT Error]', err);
        setVoiceInputNotice(err.message || 'Microphone permission is required for voice input.');
        setTimeout(() => setVoiceInputNotice(null), 5000);
      }
    } else if (recordingState === 'recording') {
      setRecordingState('processing');
      try {
        const result = await sarvamRecorderRef.current.stopAndTranscribe(selectedLanguage);
        if (result && result.transcript) {
          setInputText(result.transcript);
          setRecordingState('idle');
        } else {
          setRecordingState('idle');
          setVoiceInputNotice('Could not recognize speech — please try speaking again.');
          setTimeout(() => setVoiceInputNotice(null), 4000);
        }
      } catch (err) {
        setRecordingState('idle');
        console.warn('[Sarvam STT Transcribe Error]', err);
        setVoiceInputNotice(err.message || 'Voice service is temporarily unavailable. You can continue using text chat.');
        setTimeout(() => setVoiceInputNotice(null), 5000);
      }
    }
  };

  // Handle Sarvam Text-to-Speech Audio Playback
  const handlePlaySarvamTTS = async (msgId, textToPlay) => {
    ...
    await playSarvamTTS({
      ...
      onError: (err) => {
        setPlayingMsgId(null);
        setVoiceInputNotice(err.message || 'Unable to play voice output.');
        setTimeout(() => setVoiceInputNotice(null), 4000);
      }
    });
  };

  // Web Speech Recognition (Fallback Voice Input)
  const toggleListening = () => {
    if (sarvamAvailable) {
      handleSarvamRecordToggle();
      return;
    }
  ...
  const speakText = (textToSpeak, msgId = null) => {
    if (sarvamAvailable && msgId) {
      handlePlaySarvamTTS(msgId, textToSpeak);
      return;
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-6xl mx-auto w-full px-2 sm:px-4 py-2">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-3xl ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                          ...
                          {msg.richContent.miniForecast.map((card, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="text-xs font-bold text-slate-800">{msg.richContent ? card.value : card.value}</div>
                              </div>
                            );
                          })}
                        </div>
              ...
              {/* Source Line, Timestamp & Sarvam TTS Play Button */}
              {msg.sender === 'ai' && (
                <div className="flex items-center justify-between gap-2 text-[10px] text-slate-400 mt-1 pl-1 font-medium">
                  ...
                  {/* Sarvam AI TTS Speaker Button */}
                  <button
                    type="button"
                    onClick={() => handlePlaySarvamTTS(msg.id, msg.text)}
                    className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[10px] font-semibold transition-all cursor-pointer ${
                      playingMsgId === msg.id
                        ...
                        : 'bg-sky-50 dark:bg-sky-950/80 text-sky-700 dark:text-sky-300 border-sky-200/80 hover:bg-sky-100'
                    }`}
                    title="Play response using Sarvam Text-to-Speech"
                  >
                    {playingMsgId === msg.id ? (
                      <>
      ...
      {(recordingState !== 'idle' || isListening || voiceInputNotice) && (
          ...
          {recordingState === 'recording' || isListening ? (
            <div className="bg-rose-50 border border-rose-200 text-rose-700 px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center justify-between shadow-2xs animate-pulse">
              <div className="flex items-center gap-2">
                ...
                <span>🔴 Listening... speak your question (Sarvam STT active)</span>
              </div>
            ...
            <div className="bg-sky-50 border border-sky-200 text-sky-700 px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-2 shadow-2xs">
              ...
              <span>⏳ Processing audio with Sarvam Speech-to-Text...</span>
            </div>
          ) : (
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-2 shadow-xs shrink-0">
        ...
        >
          ...
          <button
            type="button"
            onClick={sarvamAvailable ? handleSarvamRecordToggle : toggleListening}
        ...
        <div className="text-[10px] text-slate-400 text-center mt-1.5 pt-1 border-t border-slate-100">
          WeatherGPT combines high-resolution IMD radar data with ECMWF-HRES ensembles & Sarvam AI Multilingual Voice.
        </div>
      </div>
    </div>
  );
