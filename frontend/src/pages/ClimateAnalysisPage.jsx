    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-200/80">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 tracking-tight">Climate Analysis &amp; Historical Anomaly</h1>
            <p className="text-xs text-slate-500">
              5-year historical temperature trends and monthly temperature anomaly for <span className="font-semibold text-slate-700">{currentLocation?.name || 'Selected Location'}</span>.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-xs font-semibold text-slate-600 hover:text-sky-600 hover:border-sky-300 transition-all cursor-pointer disabled:opacity-50"
        >
      ...
      {isLoading ? (
        <div className="bg-white border border-slate-200/80 rounded-2xl p-12 flex flex-col items-center justify-center gap-3 shadow-xs">
          <div className="w-10 h-10 rounded-xl bg-sky-50 text-sky-600 flex items-center justify-center shadow-xs">
            <Loader2 className="w-5 h-5 animate-spin" />
          </div>
          <div className="text-center">
            <div className="text-sm font-bold text-slate-800">Fetching Historical Climate Archive...</div>
            <div className="text-xs text-slate-400 mt-0.5">Analyzing 5-year Open-Meteo weather records for {currentLocation?.name || 'location'}</div>
          </div>
        </div>
      ) : error ? (
        <div className="bg-red-50/80 border border-red-200 rounded-2xl p-6 text-center space-y-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-red-800">Failed to Load Climate Data</h3>
